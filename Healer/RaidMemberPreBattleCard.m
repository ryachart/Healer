...
-(id)initWithFrame:(CGRect)frame count:(NSInteger)cnt andRaidMember:(RaidMember *)member{
    if (self = [super initWithColor:ccc4(255, 255, 255, 255)]){
        ...
        CCLabelTTF *healthLabel = [CCLabelTTF labelWithString:[NSString stringWithFormat:@"Health: %i", raidMember.maximumHealth] dimensions:CGSizeMake(200, 50) alignment:UITextAlignmentLeft fontName:@"Arial" fontSize:24];
        [healthLabel setColor:ccBLACK];
        [healthLabel setPosition:ccp(100, 78)];
        [detailBackground addChild:healthLabel];
        
        CCLabelTTF *DPSLabel = [CCLabelTTF labelWithString:[NSString stringWithFormat:@"DPS: %1.2f", raidMember.dps] dimensions:CGSizeMake(200, 50) alignment:UITextAlignmentLeft fontName:@"Arial" fontSize:24];
        [DPSLabel setColor:ccBLACK];
        [DPSLabel setPosition:ccp(100, 58)];
        [detailBackground addChild:DPSLabel];
        
        CCLabelTTF *descLabel = [CCLabelTTF labelWithString:self.raidMember.info dimensions:CGSizeMake(200, 80) alignment:UITextAlignmentLeft fontName:@"Arial" fontSize:12];
        [descLabel setColor:ccBLACK];
        [descLabel setPosition:ccp(100, 20)];
        [detailBackground addChild:descLabel];
        
        CCLabelTTF *classNameLabel = [CCLabelTTF labelWithString:self.raidMember.title dimensions:CGSizeMake(95, 48) alignment:UITextAlignmentLeft fontName:@"Arial" fontSize:16];
        [classNameLabel setPosition:ccp(50, 78)];
        [countBackground addChild:classNameLabel];
        
        CCLabelTTF *countLabel = [CCLabelTTF labelWithString:[NSString stringWithFormat:@"%i", count] dimensions:CGSizeMake(50, 50) alignment:UITextAlignmentLeft fontName:@"Arial" fontSize:24];
