...
import type { RegistryInput, CombatEvent, CombatStateSnapshot, EncounterResolutionSnapshot } from "./engine/types.js";
...
interface GameplaySummary {
  preview: PrebattleViewModel;
  finalState: CombatStateSnapshot;
...
function resetProfile(state: AppState): void {
...
function healthPercent(health: number, maximumHealth: number): number {
  if (maximumHealth <= 0) {
    return 0;
  }
...
function priorityForEffect(event: CombatStateSnapshot["effects"][number]): number {
  let priority = 0;
...
function setSelectedSpells(state: AppState, appRoot: HTMLElement, spellIds: string[]): void {
...
function runAutoplayEncounter(state: AppState, level: number): GameplaySummary {
  ...
  const resolution = resolveEncounterOutcome(
    state.registry,
    combatState,
    createEncounterProgressionInput(state.profile),
  );
...
function renderGameplay(content: HTMLElement, state: AppState, appRoot: HTMLElement): void {
  ...
  playerCard.append(element(
    "p",
    "detail-card__text",
    `${formatNumber(Math.trunc(gameplay.finalState.player.health))}/${formatNumber(Math.trunc(gameplay.finalState.player.maximumHealth))} HP`,
  ));
  playerCard.append(element(
    "p",
    "detail-card__text",
    `${formatNumber(Math.trunc(gameplay.finalState.player.energy))}/${formatNumber(Math.trunc(gameplay.finalState.player.maximumEnergy))} Energy`,
  ));
  playerCard.append(element(
    "p",
    "detail-card__text",
    gameplay.finalState.player.casting
...
function renderPostbattle(content: HTMLElement, state: AppState, appRoot: HTMLElement): void {
  const gameplay = state.gameplay;
  if (!gameplay) {
    state.screen = "map";
...
function renderAcademy(content: HTMLElement, state: AppState, appRoot: HTMLElement): void {
  const section = element("section", "panel");
...
function renderArmory(content: HTMLElement, state: AppState, appRoot: HTMLElement): void {
  const section = element("section", "panel");
...
function renderTalents(content: HTMLElement, state: AppState, appRoot: HTMLElement): void {
  const section = element("section", "panel");
...
function renderSettings(content: HTMLElement, state: AppState, appRoot: HTMLElement): void {
  const section = element("section", "panel");
