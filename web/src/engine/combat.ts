...
function copyState(state: CombatStateSnapshot): CombatStateSnapshot {
  return {
    ...state,
    replay: { ...state.replay },
    encounter: {
...
function applySpellCooldown(spell: CombatPlayerSpellSnapshot): void {
...
function disableFriendly(target: FriendlyCombatant): void {
...
export function beginPlayerCast(state: CombatStateSnapshot, request: PlayerCastRequest): CombatUpdateResult {
  const next = copyState(state);
